...
เนื่องจากคอลัมน์ $a \oplus b$ และ $a \cdot b' + a' \cdot b$ มีค่าเท่ากันทุกแถว ดังนั้น $a \oplus b = a \cdot b' + a' \cdot b$
\end{ex}
...
วิธีทำ: เราจะใช้ทฤษฎีบทต่างๆ ที่เรียนมาข้างต้นในการลดรูปอย่างเป็นระบบ โดยจะอธิบายเหตุผลในแต่ละขั้นตอนอย่างชัดเจน

\begin{align*}
f &= (a + b) \cdot (a' + c) + (b + c) \\
  &= \underbrace{(a + b) \cdot a'}_{\text{พจน์ที่ 1}} + \underbrace{(a + b) \cdot c}_{\text{พจน์ที่ 2}} + (b + c) \quad \text{(Distributive: } x \cdot (y + z) = x \cdot y + x \cdot z \text{)} \\
  &= \underbrace{a \cdot a'}_{\text{พจน์ที่ 1}} + \underbrace{b \cdot a'}_{\text{พจน์ที่ 2}} + \underbrace{a \cdot c}_{\text{พจน์ที่ 3}} + \underbrace{b \cdot c}_{\text{พจน์ที่ 4}} + b + c \quad \text{(Distributive อีกครั้ง)} \\
  &= 0 + b \cdot a' + a \cdot c + b \cdot c + b + c \quad \text{(Complement: } a \cdot a' = 0 \text{)} \\
  &= b \cdot a' + a \cdot c + \underbrace{b \cdot c + b}_{\text{พจน์ที่ 5}} + c \\
  &= b \cdot a' + a \cdot c + b \cdot (1 + c) + c \quad \text{(Distributive ย้อนกลับ)} \\
  &= b \cdot a' + a \cdot c + b \cdot 1 + c \quad \text{(Domination: } 1 + c = 1 \text{)} \\
  &= b \cdot a' + a \cdot c + b + c \\
  &= \underbrace{b \cdot a' + b}_{\text{พจน์ที่ 6}} + a \cdot c + c \\
  &= b \cdot (a' + 1) + a \cdot c + c \quad \text{(Distributive ย้อนกลับ)} \\
  &= b \cdot 1 + a \cdot c + c \quad \text{(Domination: } a' + 1 = 1 \text{)} \\
  &= b + a \cdot c + c \\
  &= b + \underbrace{a \cdot c + c}_{\text{พจน์ที่ 7}} \\
  &= b + c \cdot (a + 1) \quad \text{(Distributive ย้อนกลับ)} \\
  &= b + c \cdot 1 \quad \text{(Domination: } a + 1 = 1 \text{)} \\
  &= b + c
\end{align*}

ดังนั้น $f = b + c$ ซึ่งเป็นรูปแบบที่ง่ายกว่านิพจน์เดิมมาก

ตรวจสอบความถูกต้องโดยใช้ truth table สำหรับทุกกรณีของ $a, b, c$:
