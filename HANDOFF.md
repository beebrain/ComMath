...
อัปเดต: 2026-08-09 · branch: `book-cleanup-2568` · commit ล่าสุด: `1a43beb` (commit ครบแล้ว)
...
- **เครื่องมือใหม่** `tools/term_audit.py` (ตรวจศัพท์ขัดกันกับ `glossary.tsv` 282 คำ), `tools/check_intro.py` (บังคับกฎเกริ่นนำ 200 ตัวอักษร คืน exit 1 เมื่อพบจุดผิด), `tools/build_glossary.py`, `build.sh`
...
### บั๊กที่ต้องระวังซ้ำ (พบใน ch2 เกือบทำให้เฉลยทั้งบทใช้ไม่ได้)
`\setcounter{enumi}` ในบล็อกเฉลยถูกตั้งตามลำดับ *บล็อกเฉลย* ไม่ใช่ตามเลข *โจทย์* ทำให้เลขเฉลยเหลื่อมจากเลขโจทย์ตั้งแต่ข้อ 7 ถึง 19 · **มองด้วยตาไม่เห็น เพราะไฟล์ `.tex` ดูเรียบร้อยดี ปรากฏเฉพาะตอนเรียงพิมพ์** · ก่อนปิดทุกบทที่แบ่งเฉลยเป็นส่วน ให้รันสคริปต์เทียบจำนวนบล็อกเฉลยกับจำนวน `\begin{exercise}` และเทียบจำนวนข้อย่อยของแต่ละคู่ · ch4 ch5 ch6 ใช้รูปแบบเดียวกัน จึงมีโอกาสเลขเหลื่อมด้วย
...
- **ตรวจ `\setcounter{enumi}` ของบล็อกเฉลยใน ch4 ch5 ch6 ก่อนอย่างอื่น** เป็นบั๊กที่พบใน ch2 และรูปแบบเดียวกันน่าจะพลาดซ้ำ
