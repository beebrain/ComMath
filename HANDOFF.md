...
อัปเดต: 2026-08-07 · branch: `book-cleanup-2568` · commit ล่าสุด: `b63ac27` (commit ครบแล้ว)
...
## next step
- **ไม่มีงานค้างที่บล็อกอยู่** ตัวชี้วัดทั้งเล่มสะอาดหมด (ดูหัวข้อ "รอบ 2026-08-07")
- 4 จุดที่แก้เนื้อหาผิดใน ch6 **ผู้เขียนรีวิวและอนุมัติแล้ว 2026-08-07** ตรวจซ้ำด้วยการไล่ค่าอินพุตทุกกรณี
  ยืนยันว่าเฉลยใหม่ถูกทั้งหมดและเฉลยเดิมผิดจริง ไม่ต้องทำอะไรต่อ
- งานที่ทำได้ถ้าต้องการต่อ (ไม่เร่ง)
  1. Underfull hbox 21 จุด — คำเตือนการจัดบรรทัด ต้องเกลาถ้อยคำทีละจุด
  2. เพิ่มการอ้างอิง — ทั้งเล่มมี \cite เพียง 30 จุดใน 253 หน้า (ch4 น้อยสุด 3 จุด)
     เกณฑ์เอกสารคำสอนของ ก.พ.อ. ระบุว่าต้องมีการอ้างอิงเพื่อขยายความที่มาของสาระ
  3. รูปประกอบไม่สมดุล — ch6 มี 1 รูปทั้งบท ขณะที่ ch2 มี 8 รูป
  4. `tikztest.tex` ที่ root ยังอ้างชื่อรูป 19–24 ที่ลบไปแล้ว (ไม่ include ใน main.tex จึงไม่กระทบ build)
