...
ในการศึกษาพีชคณิตบูลีน เราจะใช้ตารางค่าความจริงเป็นเครื่องมือสำคัญในการแสดงผลลัพธ์ของการดำเนินการสำหรับทุกกรณีของค่าอินพุต สำหรับตัวแปร $n$ ตัวจะมี $2^n$ แถวในตารางค่าความจริง ดังนั้นสำหรับ 2 ตัวแปรจะมี 4 แถว และสำหรับ 3 ตัวแปรจะมี 8 แถว ความสามารถในการอ่านและเขียนตารางค่าความจริงอย่างคล่องแคล่วจึงเป็นทักษะพื้นฐานที่จำเป็นสำหรับทุกผู้ศึกษาเกี่ยวกับวงจรดิจิทัล
...
\begin{enumerate}
    ...
    \item $a \cdot (b \oplus c) = a \cdot b \oplus a \cdot c$ \hfill (การแจกแจงของ AND บน XOR)
...
กฎของพีชคณิตบูลีน (Boolean identities) เป็นความสัมพันธ์ที่เป็นจริงสำหรับทุกค่าของตัวแปรบูลีน กฎเหล่านี้สามารถพิสูจน์ได้โดยการตรวจสอบทุกกรณีของค่าตัวแปรผ่านตารางค่าความจริง หรือโดยการใช้กฎอื่นที่ได้พิสูจน์แล้ว กฎเหล่านี้มีความสำคัญอย่างยิ่งในการลดรูปนิพจน์บูลีน ซึ่งเป็นพื้นฐานของการออกแบบวงจรดิจิทัลที่มีประสิทธิภาพ การลดรูปนิพจน์บูลีนที่ซับซ้อนให้เป็นรูปแบบที่ง่ายกว่าจะช่วยลดจำนวน logic gates ที่ต้องใช้ ทำให้วงจรมีขนาดเล็กลง ประหยัดพลังงานมากขึ้น และทำงานได้เร็วขึ้น~\parencite{mano2018digital}
...
ก่อนเข้าสู่กฎแต่ละข้อ ให้สังเกตว่ากฎเหล่านี้มาเป็นคู่เสมอ เช่น $x + 0 = x$ มาคู่กับ $x \cdot 1 = x$ และ $x + 1 = 1$ มาคู่กับ $x \cdot 0 = 0$ ที่เป็นเช่นนี้ไม่ใช่ความบังเอิญ แต่เป็นผลจากสมบัติเชิงโครงสร้างของสัจพจน์ที่นิยามพีชคณิตบูลีนไว้ตั้งแต่ต้น กล่าวคือสัจพจน์ทุกข้อถูกเขียนไว้เป็นคู่ที่สมมาตรกันระหว่าง $+$ กับ $\cdot$ อยู่แล้ว
...
ประโยชน์ในทางปฏิบัติคือ \textbf{พิสูจน์กฎหนึ่งข้อแล้วได้อีกข้อฟรี} จึงเป็นเหตุผลว่าทำไมกฎในบทนี้จึงนำเสนอเป็นคู่ตลอด และเหตุใดตารางที่~\ref{tab:boolean-identities} จึงจัดกฎคู่ทวิภาคไว้ในแถวเดียวกัน
\end{principle}
...
\begin{proof}
พิสูจน์โดยใช้ตารางค่าความจริง พิจารณาทุกกรณีของ $a$ และ $b$:
...
สาระสำคัญของกฎทั้งสองข้อคือ การนิเสธนิพจน์ทั้งก้อนเท่ากับการนิเสธตัวแปรทีละตัวแล้วสลับตัวดำเนินการระหว่าง $+$ กับ $\cdot$ รูปที่~\ref{fig:boolean-demorgan-flow} แสดงการสลับนี้ในรูปแผนภาพ โดยอ่านจากซ้ายไปขวาได้ว่าประตูที่ต่อนิเสธไว้ที่ขาออกเทียบเท่ากับประตูอีกชนิดหนึ่งที่ต่อนิเสธไว้ที่ขาเข้าทุกขา
...
\label{sec:dont-care}
ในงานออกแบบจริง ชุดข้อมูลเข้าบางชุดไม่มีทางเกิดขึ้นได้เลยตามข้อกำหนดของระบบ ค่าของฟังก์ชันที่ชุดข้อมูลเข้าเหล่านั้นจึงกำหนดเป็นอะไรก็ได้โดยไม่กระทบการทำงาน เรียกว่าค่าไม่สนใจ และเขียนแทนด้วยสัญลักษณ์ $d$ ในแผนที่ ผู้ออกแบบจึงเลือกให้เป็น 0 หรือ 1 ก็ได้ตามที่ทำให้ผลลัพธ์ประหยัดที่สุด~\parencite{mano2018digital}
...
หัวข้อนี้นำวิธีแผนที่คาร์นอที่ศึกษาใน~\autoref{sec:karnaugh-map} มาใช้กับโจทย์ออกแบบวงจรที่มีข้อกำหนดมาจากงานจริง โจทย์กลุ่มนี้มักมาพร้อมค่าไม่สนใจตามที่กล่าวไว้ใน~\autoref{sec:dont-care} เพราะข้อกำหนดของระบบมักตัดชุดข้อมูลเข้าบางชุดออกไปตั้งแต่ต้น ตัวอย่างต่อไปนี้เป็นกรณีที่ฟังก์ชันระบุค่าครบทุกกรณี จึงลดรูปด้วยการจับกลุ่มตามปกติ~\parencite{roth2021logic}
...
วงจรบวกครึ่งและวงจรบวกเต็มใน~\autoref{sec:adders} นำมาต่อเรียงกันเพื่อบวกเลขฐานสองหลายบิต โดยส่งบิตทดจากหลักหนึ่งไปเป็นข้อมูลเข้าของหลักถัดไป โครงสร้างนี้เป็นแกนของหน่วยคำนวณและตรรกะในหน่วยประมวลผลกลาง~\parencite{wakerly2018digital} ตัวอย่างต่อไปนี้แสดงการสร้างวงจรบวกเต็มจากวงจรบวกครึ่งสองชุด รูปที่~\ref{fig:adders-comparison} วางวงจรทั้งสองไว้เทียบกัน จุดต่างที่ควรสังเกตคือขาเข้าที่สามของวงจรบวกเต็มซึ่งรับบิตทดจากหลักก่อนหน้า และประตู OR ที่รวมบิตทดจากวงจรบวกครึ่งทั้งสองชุด
